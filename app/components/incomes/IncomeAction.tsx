...
import { formatRupiah, formatDateDisplay } from '@/app/utils/helper';
...
export const IncomeActions: React.FC<IncomeActionsProps> = ({
    entriesPerPage,
    onEntriesChange,
    onAddClick,
    dropdownOpen,
    ...
    const handleExport = () => {
        if (incomes && incomes.length > 0) {
            const formattedData = incomes.map((item, index) => ({
                'No': index + 1,
                'Bulan': formatDateDisplay(item.MonthlyFinance.month),
                'Sumber': item.source,
                'Deskripsi': item.description,
                'Jumlah': formatRupiah(item.amount),
                'Tanggal Pemasukan': new Date(item.incomeDate).toLocaleDateString('id-ID', {
                    day: '2-digit',
                    month: 'long',
                    year: 'numeric'
                }),
            ...
            exportToExcel(formattedData, 'Data Pemasukan');
        } else {
