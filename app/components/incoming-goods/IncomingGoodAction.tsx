...
interface IncomingGoodsActionsProps {
...
export const IncomingGoodsActions: React.FC<IncomingGoodsActionsProps> = ({
    entriesPerPage,
    onEntriesChange,
    onAddClick,
    dropdownOpen,
    ...
    const handleExport = () => {
        if (incomingGoods && incomingGoods.length > 0) {
            const formattedData = incomingGoods.map((item, index) => ({
                'No': index + 1,
                'Nama Barang': item.inventory?.name || 'N/A',
                ...
                'Kondisi': ConditionLabel[item.condition],
            ...
            exportToExcel(formattedData, 'Data Barang Masuk');
        } else {
