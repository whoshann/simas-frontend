...
export const generateAchievementReport = (achievementData: Achievements[]): jsPDF => {
    const doc = new jsPDF();
    
    // Tambahkan kop surat hanya di halaman pertama
    addHeaderTemplate(doc);
    
    // Tambahkan judul
    doc.setFontSize(16);
    doc.setFont('Times New Roman', 'bold');
    doc.text('LAPORAN PRESTASI SISWA', doc.internal.pageSize.width / 2, 45, { align: 'center' });
    
    // Kelompokkan data berdasarkan kelas
    const groupedData = achievementData.reduce((acc, item) => {
        const className = item.class?.name || 'Tidak Ada Kelas';
        if (!acc[className]) {
            acc[className] = [];
        }
    ...
    grades.forEach(grade => {
        ...
        if (classesInGrade.length > 0) {
            ...
            classesInGrade.forEach((className) => {
                ...
                const tableData = classData.map((item, index) => [
                    ...
                    item.typeOfAchievement === 'Non_Academic' ? 'Non Akademik' : 'Akademik',
                    format(new Date(item.achievementDate), 'dd MMMM yyyy', { locale: id })
                ]);

                // Buat tabel untuk kelas ini
                doc.autoTable({
                    startY: startY + 5,
                    head: [['No', 'Nama Siswa', 'Kelas', 'Nama Prestasi', 'Nama Kompetisi', 'Kategori', 'Tanggal']],
                    body: tableData,
                    styles: {
